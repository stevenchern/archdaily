...
def get_projects
  ...
  count = 1
  @projects = []
  JSON.parse(project_json.to_json, object_class: OpenStruct).each do |hash|
    @projects << instance_variable_set("@project_#{count}", Project.new(hash))
    count += 1
  end
end
...
def most_bookmarked_projects(place, month)
  get_projects
  @projects.select { |project| project.count_bookmarks_in_month(month) > 0 && project.sites.include?(place) }
...
def bookmarks_per_month(month)
  get_projects
  @projects.map { |project| project.count_bookmarks_in_month(month) }.sum
end